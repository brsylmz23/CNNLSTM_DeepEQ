...
# test = True
test = False
working_directory = r"D:\Baris\codes\LSTM_SP_EQT"
...
wandb.login()
##############################################################################   
def main(lat,lon,signaltime,transferpath,hidden, lr, LSTM_split):
...
############################################################################## 
    ...
    model = CNNLSTM(3,hidden,2,1).to(device)
...
for l in range(len(LSTM_split)):
    for i in range(len(lr)):
        for k in range(len(hidden)):
            ...
            main(lat[0],lon[0],signaltime[0],transferpath,hidden[k], lr[i], LSTM_split[l])
            end_time = time.time()
