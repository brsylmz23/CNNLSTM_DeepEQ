            ...
            for i in range(len(gt[0])): # Per column of gt
                for mb in range(len(gt)): # Per elements in a  --Z loop iterates for 64 times if batchSize is 64                    
                    ...
                    if kwargs.get("gtnorm"):
                        ...
                        if phase == "training":
                            wandb.log({"Vs30_pred_train": np.mean(np.abs(float(predicted_val.item() - true_val.item())))})
                        elif phase =="validation":
                            wandb.log({"Vs30_pred_val": np.mean(np.abs(float(predicted_val.item() - true_val.item())))})
