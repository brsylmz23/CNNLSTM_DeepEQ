...
def datasetCreator(**kwargs):
    ...
    **kwargs: Keyword arguments specifying the dataset details
    ...
    for b in os.listdir(fname)[0:len(os.listdir(fname))]: 
        ...
        if kwargs['SP']:
            Ptime= dataset['EQ'][0][0]['anEQ']["Ptime"][0][0][0][0]
            Stime =dataset['EQ'][0][0]['anEQ']["Stime"][0][0][0][0]
        else:
            Ptime= 0
            Stime =0
            
        epicenterDepth = float(dataset['EQ'][0][0]['anEQ']['depth'][0][0][0][0])/kwargs.get('km2meter')
        Vs30 = dataset['EQ'][0][0]['anEQ']["Vs30"][0][0][0][0]  
        ...
        if kwargs['SP']:
            signal_acc = dataset['EQ'][0][0]['anEQ']['Accel'][0][0] 
            sp_signal = np.zeros(len(signal_acc))
            for i in range(len(signal_acc)):
                if int(dataset['EQ'][0][0]['anEQ']["Ptime"][0][0][0][0]*kwargs['fs']) <= i <= int(dataset['EQ'][0][0]['anEQ']["Stime"][0][0][0][0]*kwargs['fs']):
                    sp_signal[i] = 1
        ...
        gettingfiles, groundTruth, input_signal,  magnitude_val, stat2epi, signal_list, epicenter_depth, lats, longs, stationco , epicentraldist ,altitudes , stat_id , sP_3_channel, tpga_ind ,stat_info, skipped_file, Vs30,Ptime, Stime = loader_helper(
            b, signal, numofData, epicenterLat,epicenterLon, epicenterDepth, 
            ...
            skipped_file, Vs30, Ptime, Stime, **kwargs)
    ...
    attributes = {'Matfiles': gettingfiles, 'Signal': signal_list, 'station_km':stat2epi, 'Latitudes': lats,'trStds':trStds, 
                  'Longitudes': longs, 'Depths': epicenter_depth, 'Station_Coordinates': stationco, 'Epicentral_distance(epi2loc)': epicentraldist, 
                  'Magnitudes': magnitude_val, 'Altitudes': altitudes, 'Station ID': stat_id, 
                  'groundTruth': groundTruth, 'stat_info': stat_info, "tpga_ind": tpga_ind,"Vs30":Vs30, 'skipped_file':skipped_file}                                
...
class structureData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sig_to_return = torch.tensor(self.signals[index]).clone()# 19x51x3
...
def loader_helper(b, signal, numofData, epicenterLat, epicenterLon, epicenterDepth, 
                  stationLat, stationLon, stationcoor, gettingfiles, groundTruth, input_signal, 
                  magnitude_val, stat2epi, signal_list, epicenter_depth, lats, longs, stationco, 
                  epicentraldist, altitudes, stat_id, sP_3_channel, tpga_ind, stat_info,
                  skipped_file, Vs30, Ptime, Stime, **kwargs):
    ...
    try: 
        ...
        p_index = np.argmax(signal,axis=0)
        median_index = np.max(p_index)/kwargs.get('fs')

        ##spt to dis yoksa ekleme
        ...
        if (epi2loc > float(kwargs.get('radius'))):
            ...
            if (station_km > int(kwargs.get('stat_dist'))):
                ...
                if numofData > (int(kwargs.get('signaltime'))*kwargs.get('fs'))-1:
                    # Check if epicenter depth is less than desired depth value
                     if epicenterDepth < float(kwargs.get('depth')):
                         start_index = median_index*kwargs.get('fs')- int((kwargs.get('signaltime'))/2*kwargs.get('fs'))
                         
                         
                         start_index = max(0, start_index)
                         end_index = start_index + int(kwargs.get('signaltime'))*kwargs.get('fs')
                         required_length = (int(kwargs.get('signaltime'))*kwargs.get('fs'))
                         actual_length = end_index - start_index
                         if end_index <= numofData:
                             
                             if actual_length == required_length:
                                # Eğer uzunluklar eşitse, sinyal bu aralıkta işlenir
                                signal = signal[int(start_index):int(end_index), :]
                ...
                # Check if median index(tpga) is less than or equal to desired signal duration
                                ...
                                #         signal = signal[median_index*kwargs.get('fs') - int((kwargs.get('signaltime'))/2*kwargs.get('fs')) :median_index*kwargs.get('fs') +  int((kwargs.get('signaltime'))/2*kwargs.get('fs')),:]
                                ...
                                signal = signal.reshape((int(kwargs.get('signaltime')) )*kwargs.get('fs'),1,4)    
                                ...
                                counter += 1
                                stat_id.append(b[-8:-4])
                             else:
                                   # Uzunluklar eşit değilse hata durumu
                                 gettingfiles.remove(b)
                                 groundTruth.pop()
    ...
    return gettingfiles, groundTruth, input_signal,  magnitude_val, stat2epi, signal_list, epicenter_depth, lats, longs, stationco, epicentraldist, altitudes, stat_id, sP_3_channel, tpga_ind, stat_info, skipped_file, Vs30, Ptime, Stime
