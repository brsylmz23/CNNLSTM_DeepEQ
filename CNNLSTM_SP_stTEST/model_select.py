...
from utils import init_weights, debug_point
...
def model_select(signal_height,signal_width,device,**kwargs):
    ...
    if kwargs.get("model_select")==["TCN"]:
        if kwargs.get("Transfer_model") and kwargs.get("Transfer_encoder"):
            ...
            model = TCN(nb_filters=20, kernel_size=(6,1), nb_stacks=1, dilations=[2 ** i for i in range(11)], use_skip_connections=True, output_size = len(kwargs["gt_select"]),  drop_rate = kwargs.get("dropout_rate"), return_sequences=False).to(device)
    else:
        if kwargs.get("Transfer_model") and kwargs.get("Transfer_encoder"):
            encoder_model = ResNet(signal_height, signal_width,**kwargs).to(device)
            decoder_model= ResNet(signal_height, signal_width,**kwargs).to(device)        
        else:    
    ...
    if kwargs.get("Transfer_model"):
        if kwargs.get("Transfer_encoder"):
            if kwargs.get("model_select")==["TCN"]:
                ...
                encoder_model.eval()
                decoder_model.apply(init_weights)
                model = ResNet(encoder_model, decoder_model)
                return model,decoder_model
        else:    
            state_dict = torch.load(os.path.join(kwargs.get("transfer_path")) + "finalModel_state_dict.pt")
