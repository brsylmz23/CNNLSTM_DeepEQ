...
def true_vs_predicted_plot(trn_true,trn_pred, val_true,val_pred, tst_true, tst_pred, s_path, title):
    ...
    # Saving directory
    plt.rcParams["savefig.directory"] = os.chdir(s_path)
...
def plot_1d_histograms(depth_loss, metric_dist, s_path, suptitle, title1, title2):
    ...
    fig, [ax,ax1] = plt.subplots(1,2, dpi=1200)
    plt.rcParams["savefig.directory"] = os.chdir(s_path)
    plt.rcParams["figure.autolayout"] = True
    fig.suptitle(suptitle)
    counts, bins, bars = ax.hist(depth_loss, edgecolor='black', bins=np.arange(-70, 70,w))
    ...
    ax1.hist(metric_dist, bins=np.arange(-70, 70,w))
...
def plot_2d_histograms(depth_loss, metric_dist, true_depth, r, s_path):   
    ...
    fig, [ax,ax1,ax2] = plt.subplots(1,3, figsize=(8, 3), constrained_layout=True, dpi=1200)
    plt.rcParams["savefig.directory"] = os.chdir(s_path)
...
def PlotExperiment(attributes, plotargs):
    ...
    if "Vs30" in plotargs.get("gt_select"):
        ...
        plot_1d_histograms(plotargs.get('Vs30_diff'),  plotargs.get('Vs30_diff'),
                            s_path,
                            suptitle='Vs30_diff Histogram',
                            title1='Vs30_diff (m/s)',
                            title2='blank')
        
        plot_1d_histograms(plotargs.get('Vs30_percentage'),  plotargs.get('Vs30_percentage'),
                            s_path,
                            suptitle='Vs30_percentage Histogram',
                            title1='Vs30_percentage (%)',
                            title2='blank')
...
def StationFigure(attributes, plotargs, add_Margin=True):
    ...
    for i, station_name in enumerate(test_stations_unique_names):
        ...
        # Plot parameters
        plt.rcParams["savefig.directory"] = os.chdir(os.path.join((plotargs.get('save_path')), 'figs'))
...
def StationFigure_test_stations_training_data(attributes, plotargs, add_Margin=True):
    ...
    for i, station_name in enumerate(test_stations_unique_names):
        ...
        for index in sig_indices_of_this_station_in_test_set:
            ...
            # plot points 
            plt.plot(true_lon, true_lat, marker='x', markersize=7, markerfacecolor="None", markeredgecolor='navy', linestyle="None")
            plt.plot(predicted_lon, predicted_lat, marker='o', markersize=7, markerfacecolor="None", markeredgecolor='navy', linestyle="None")
            
            # plot lines
            plt.plot([predicted_lon, true_lon], [predicted_lat, true_lat], linestyle='-', color='cornflowerblue',alpha=0.8, linewidth=0.5)
        ...
        for index in sig_indices_of_this_station_in_validation_set:
            ...
            # plot points 
            plt.plot(true_lon, true_lat, marker='x', markersize=7, markerfacecolor="None", markeredgecolor='orange', linestyle="None")
            plt.plot(predicted_lon, predicted_lat, marker='o', markersize=7, markerfacecolor="None", markeredgecolor='orange', linestyle="None")
            
            # plot lines
            plt.plot([predicted_lon, true_lon], [predicted_lat, true_lat], linestyle='-', color='moccasin',alpha=0.8, linewidth=0.5)
        ...
        for index in sig_indices_of_this_station_in_training_set:
            ...
            # plot points 
            plt.plot(true_lon, true_lat, marker='x', markersize=7, markerfacecolor="None", markeredgecolor='green', linestyle="None")
            plt.plot(predicted_lon, predicted_lat, marker='o', markersize=7, markerfacecolor="None", markeredgecolor='green', linestyle="None")
            # plot lines
            plt.plot([predicted_lon, true_lon], [predicted_lat, true_lat], linestyle='-', color='thistle',alpha=0.8, linewidth=0.5)
            # if no legend exists, add a legend with labels for true, predicted, and station
            if not legend_exists:
                plt.scatter([], [], marker='x', facecolor='none', edgecolors='green', label='True - Training')
                plt.scatter([], [], marker='o', facecolor='none', edgecolors='green', label='Predicted - Training')
                plt.scatter([], [], marker='x', facecolor='none', edgecolors='orange', label='True - Validation')
                plt.scatter([], [], marker='o', facecolor='none', edgecolors='orange', label='Predicted - Validation')
                plt.scatter([], [], marker='p', facecolor=station_color, edgecolors=station_color, label='Station Location')
                plt.scatter([], [], marker='x', facecolor='none', edgecolors='navy', label='True - Test')
